...
class PoseResDFMNetEgoSTAN(pl.LightningModule):
    def __init__(self, **kwargs):
        ...
        self.model = get_pose_net(cfg, True, use_final=False, deconv=True)
        self.transformer = ResNetTransformerCls(in_dim=256*16, spatial_dim=16*16, seq_len=self.seq_len*16*16, dim=512, depth=3, heads=8, mlp_dim=1024, dim_head=64, dropout=self.dropout)
    ...
    def forward(self, x):
        ...
        # features = (batch_size *len_seq) x 256*4*4 x 16*16
        features = features.reshape(dim[0], dim[1], -1, 16*16)
        # features = batch_size x len_seq x 256*4*4 x 16*16
        features = features.permute(0, 1, 3, 2)
        # features = batch_size x len_seq x 16*16 x 256*4*4
        features = features.reshape(dim[0], -1, 256*4*4)
        # features = batch_size x len_seq*16*16 x 256*4*4
        features, atts = self.transformer(features)
        ## features = batch_size x 16*16 x 256*4*4
        features = features.reshape(dim[0], 16, 16, 256, 4, 4)
        features = features.permute(0, 3, 1, 4, 2, 5)
        features = features.reshape(dim[0], 256, 64, 64)
