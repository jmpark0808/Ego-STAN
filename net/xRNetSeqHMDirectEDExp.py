...
from net.transformer import ResNetTransformerCls, ResNetTransformerClsExp
import matplotlib
import pathlib


class xREgoPoseSeqHMDirectEDExp(pl.LightningModule):
    def __init__(self, **kwargs):
        ...
        self.resnet_transformer = ResNetTransformerClsExp(seq_len=self.seq_len*num_class, dim=512, depth=3, heads=8, mlp_dim=1024, dim_head=64, dropout=self.dropout)
    ...
    def forward(self, x):
        ...
        # hms = batch_size*len_seq x 17 x 47 x 47

        hms = hms.reshape(dim[0], dim[1], -1, self.heatmap_resolution[0], self.heatmap_resolution[1])
        # hms = batch_size x len_seq x 17 x 47 x 47

        hms = hms.reshape(dim[0], -1, self.heatmap_resolution[0]*self.heatmap_resolution[1])
        # hms = batch_size x len_seq*17 x 47^2
        
        hms, atts = self.resnet_transformer(hms)
        # hms = batch_size x 17 x 47^2
        hms = hms.reshape(dim[0], -1, self.heatmap_resolution[0], self.heatmap_resolution[1])
        # hms = batch_size x 17 x 47 x 47

        p3d = self.hm2pose(hms)
        # p3d = batch_size x 17 x 3
