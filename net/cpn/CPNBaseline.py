"""
    CPN Pytorch implementation for 'Cascaded Pyramid Network
    for Multi-Person Pose Estimation'

    Paper: https://arxiv.org/abs/1711.07319
    Original Code: https://github.com/GengDavid/pytorch-cpn
"""
import math
...
def CPN50(out_size, num_class, pretrained=True):
    ...
    model = CPN(
        res50, output_shape=out_size, num_class=num_class, pretrained=pretrained
    )
    return model
...
def CPN101(out_size, num_class, pretrained=True):
    ...
    model = CPN(
        res101, output_shape=out_size, num_class=num_class, pretrained=pretrained
    )
    return model
